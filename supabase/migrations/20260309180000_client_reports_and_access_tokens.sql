-- ─────────────────────────────────────────────────────────────────────────────
-- client_reports: snapshots generados desde Analytics para compartir con clientes
-- ─────────────────────────────────────────────────────────────────────────────
...
-- Miembros del workspace pueden insertar/leer/borrar
CREATE POLICY "workspace_member_client_reports"
  ON client_reports FOR ALL
  USING (is_workspace_member(auth.uid(), workspace_id));

-- Acceso público por token (para /report/:token, sin auth)
...
-- ─────────────────────────────────────────────────────────────────────────────
-- client_access_tokens: links permanentes para dashboard del cliente (/c/:token)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- Miembros del workspace pueden gestionar tokens
CREATE POLICY "workspace_member_client_access_tokens"
  ON client_access_tokens FOR ALL
  USING (is_workspace_member(auth.uid(), workspace_id));

-- Acceso público por token (Edge Function usa service_role, pero por si acaso)
