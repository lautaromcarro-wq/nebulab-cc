...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const integration of integrations) {
      ...
      if (!cred) { errors.push(`No credential for GA4 integration ${integration.id}`); continue; }

      // Refresh token if expired
      let accessToken = cred.access_token;
      if (cred.expires_at && new Date(cred.expires_at) < new Date()) {
        if (!cred.refresh_token) { errors.push(`GA4 token expired, no refresh_token`); continue; }
        const refreshed = await refreshAccessToken(clientId, clientSecret, cred.refresh_token);
        if (refreshed.error) {
          errors.push(`GA4 token refresh failed: ${refreshed.error}`);
      ...
      if (!settings?.length) {
        await supabase.from("sync_runs").insert({
          ...
          details: { skipped: true, reason: "no_enabled_properties" },
        ...
        try {
          ...
          if (rows.length === 0) {
            await supabase.from("health_events").insert({
              workspace_id: wsId, check_type: "ga4_no_data",
              severity: "info", provider: "ga4",
              message: `Property ${propertyId}: 0 rows for ${since} to ${until}`,
            });
          }
          ...
          errors.push(`Property ${propertyId}: ${errMsg}`);
        }
      }
      ...
      await supabase.from("sync_runs").insert({
        workspace_id: wsId, provider: PROVIDER, integration_id: integration.id,
        job_name: JOB_NAME, status: errors.length ? "partial" : "success",
        items_upserted: wsUpserted,
        details: { days_back: daysBack, properties: settings.map((s: any) => s.external_id), errors },
        ended_at: new Date().toISOString(), triggered_by: triggeredBy,
      });
    }
    ...
    console.error("sync-ga4-daily error:", error);
    return new Response(
